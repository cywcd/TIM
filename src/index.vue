...
export default {
  ...
  updateDestory() {
    window.removeEventListener('message', this.communicationMessageFn)
  },
  ...
  methods: {
    // 通信监听
    communicationMessageListener() {
      window.addEventListener('message', this.communicationMessageFn)
    },
    communicationMessageFn(event) {
        console.log(event.data, '--子接收父-通信消息')
    },
    initListener() {
      this.communicationMessageListener()
